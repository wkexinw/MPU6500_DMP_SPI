 ...
 * spi_write(uint8_t slave_addr, uint8_t reg_addr, uint8_t length, uint8_t const *data)
 * spi_read(uint8_t slave_addr, uint8_t reg_addr, uint8_t length, uint8_t *data)
 * delay_ms(uint32_t num_ms)
 * min(int a, int b)
 */
#define min(a,b) ((a)<(b)?(a):(b))
#define spi_write   writeBytes
#define spi_read(a,b,c,d)    (readBytes(a,b,c,d)!=-1?0:1)
...
static int set_int_enable(uint8_t enable)
{
	...
	if (st.chip_cfg.dmp_on)
	{
		if (enable)
			...
			tmp = 0x00;
		if (spi_write(st.hw->addr, st.reg->int_enable, 1, &tmp))
			return 1;
...
uint8_t mpu_reg_dump(void)
{
	...
	for (ii = 0; ii < st.hw->num_reg; ii++)
	{
		if (ii == st.reg->fifo_r_w || ii == st.reg->mem_r_w)
			continue;
		if (spi_read(st.hw->addr, ii, 1, &data))
			return 1;
...
uint8_t mpu_read_reg(uint8_t reg, uint8_t *data)
{
	...
	if (reg >= st.hw->num_reg)
		return 1;
	return spi_read(st.hw->addr, reg, 1, data);
}
...
{
	if(init()){
		printf_P("error initial bcm2835\r\n");
	}
	uint8_t data[6];
    /* Init all structs*/
    init_structs();
	...
	/* Reset device. */
	data[0] = BIT_RESET;
	if (spi_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
		return 1;
	...
	if (spi_write(st.hw->addr, st.reg->accel_cfg2, 1, data))
		return 1;
#endif
...
uint8_t mpu_lp_accel_mode(uint8_t rate)
{
	...
	if (!rate)
	{
		...
		tmp[0] = 0;
		tmp[1] = BIT_STBY_XYZG;
		if (spi_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, tmp))
			return 1;
		st.chip_cfg.lp_accel_mode = 0;
		return 0;
	}
	...
	if (rate == 1)
		...
		tmp[0] = INV_LPA_640HZ;
	if (spi_write(st.hw->addr, st.reg->lp_accel_odr, 1, tmp))
		return 1;
	tmp[0] = BIT_LPA_CYCLE;
	if (spi_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, tmp))
		return 1;
#endif
...
uint8_t mpu_get_gyro_reg(int16_t *data)
{
	...
	if (spi_read(st.hw->addr, st.reg->raw_gyro, 6, tmp))
		return 1;
...
uint8_t mpu_get_accel_reg(int16_t *data)
{
	...
	if (spi_read(st.hw->addr, st.reg->raw_accel, 6, tmp))
		return 1;
	data[0] = (tmp[0] << 8) | tmp[1];
	data[1] = (tmp[2] << 8) | tmp[3];
	data[2] = (tmp[4] << 8) | tmp[5];
...
uint8_t mpu_get_temperature(int32_t *data)
{
	...
	if (spi_read(st.hw->addr, st.reg->temp, 2, tmp))
		return 1;
	raw = (tmp[0] << 8) | tmp[1];
...
uint8_t mpu_set_accel_bias(const int32_t *accel_bias)
{
	...
	if (spi_read(st.hw->addr, 3, 3, data))
		return 1;
	fg[0] = ((data[0] >> 4) + 8) & 0xf;
	fg[1] = ((data[1] >> 4) + 8) & 0xf;
	fg[2] = ((data[2] >> 4) + 8) & 0xf;
	...
	if (spi_read(st.hw->addr, 0x06, 6, data))
		return 1;
	...
	if (spi_write(st.hw->addr, 0x06, 6, data))
		return 1;
	return 0;
}
...
uint8_t mpu_reset_fifo(void)
{
	...
	data = 0;
	if (spi_write(st.hw->addr, st.reg->int_enable, 1, &data))
		return 1;
	if (spi_write(st.hw->addr, st.reg->fifo_en, 1, &data))
		return 1;
	if (spi_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
		return 1;
	...
	else
	{
		...
		if (spi_write(st.hw->addr, st.reg->int_enable, 1, &data))
			return 1;
		if (spi_write(st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
			return 1;
	}
	return 0;
}
...
uint8_t mpu_set_gyro_fsr(uint16_t fsr)
{
	...
	if (st.chip_cfg.gyro_fsr == (data >> 3))
		return 0;
	if (spi_write(st.hw->addr, st.reg->gyro_cfg, 1, &data))
		return 1;
...
uint8_t mpu_set_accel_fsr(uint8_t fsr)
{
	...
	if (st.chip_cfg.accel_fsr == (data >> 3))
		return 0;
	if (spi_write(st.hw->addr, st.reg->accel_cfg, 1, &data))
		return 1;
...
uint8_t mpu_set_lpf(uint16_t lpf)
{
	...
	if (st.chip_cfg.lpf == data)
		return 0;
	if (spi_write(st.hw->addr, st.reg->lpf, 1, &data))
		return 1;
...
uint8_t mpu_set_sample_rate(uint16_t rate)
{
	...
	else
	{
		...
		data = 1000 / rate - 1;
		if (spi_write(st.hw->addr, st.reg->rate_div, 1, &data))
			return 1;
...
uint8_t mpu_set_sensors(uint8_t sensors)
{
	...
	if (!(sensors & INV_XYZ_ACCEL))
		data |= BIT_STBY_XYZA;
	if (spi_write(st.hw->addr, st.reg->pwr_mgmt_2, 1, &data))
	{
		st.chip_cfg.sensors = 0;
		return 1;
	}
...
uint8_t mpu_get_int_status(int16_t *status)
{
	...
	if (!st.chip_cfg.sensors)
		return 1;
	if (spi_read(st.hw->addr, st.reg->dmp_int_status, 2, tmp))
		return 1;
											...
											uint8_t *sensors, uint8_t *more)
{
	...
	if (spi_read(st.hw->addr, st.reg->fifo_count_h, 2, data))
		return 1;
	...
	if (fifo_count > (st.hw->max_fifo >> 1))
	{
		...
		if (spi_read(st.hw->addr, st.reg->int_status, 1, data))
			return 1;
	...
	if (spi_read(st.hw->addr, st.reg->fifo_r_w, packet_size, data))
		return 1;
														 ...
														 uint8_t *more)
{
	...
	if (spi_read(st.hw->addr, st.reg->fifo_count_h, 2, tmp))
		return 1;
	...
	{
		/* FIFO is 50% full, better check overflow bit. */
		if (spi_read(st.hw->addr, st.reg->int_status, 1, tmp))
			return 1;
	...
	if (spi_read(st.hw->addr, st.reg->fifo_r_w, length, data))
		return 1;
...
uint8_t mpu_set_bypass(uint8_t bypass_on)
{
	...
	if (bypass_on)
	{
		if (spi_read(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
			return 1;
		tmp &= ~BIT_AUX_IF_EN;
		if (spi_write(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
			return 1;
		delay_ms(3);
		tmp = BIT_BYPASS_EN;
		...
		if (spi_write(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
			return 1;
	}
	else
	{
...
static int get_accel_prod_shift(float *st_shift)
{
	...
	if (spi_read(st.hw->addr, 0x0D, 4, tmp))
		return 0x07;
...
static int gyro_self_test(int32_t *bias_regular, int32_t *bias_st)
{
	...
	if (spi_read(st.hw->addr, 0x0D, 3, tmp))
		return 0x07;
...
static int get_st_biases(int32_t *gyro, int32_t *accel, uint8_t hw_test)
{
	...
	data[0] = 0x01;
	data[1] = 0;
	if (spi_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, data))
		return 1;
	delay_ms(200);
	data[0] = 0;
	if (spi_write(st.hw->addr, st.reg->int_enable, 1, data))
	...
	if (spi_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
		return 1;
	if (spi_write(st.hw->addr, st.reg->i2c_mst, 1, data))
		return 1;
	if (spi_write(st.hw->addr, st.reg->user_ctrl, 1, data))
		return 1;
	...
	for (ii = 0; ii < packet_count; ii++)
	{
		...
		if (spi_read(st.hw->addr, st.reg->fifo_r_w, MAX_PACKET_LENGTH, data))
			return 1;
											...
											uint8_t *data)
{
	...
	if (spi_write(st.hw->addr, st.reg->bank_sel, 2, tmp))
		return 1;
	if (spi_write(st.hw->addr, st.reg->mem_r_w, length, data))
		return 1;
	return 0;
}
										 ...
										 uint8_t *data)
{
	...
	if (spi_write(st.hw->addr, st.reg->bank_sel, 2, tmp))
		return 1;
	if (spi_read(st.hw->addr, st.reg->mem_r_w, length, data))
		return 1;
	return 0;
}
...
#define LOAD_CHUNK  (16)
	...
	if (spi_write(st.hw->addr, st.reg->prgm_start_h, 2, tmp))
		return 1;
...
uint8_t mpu_set_dmp_state(uint8_t enable)
{
	...
	if (enable)
	{
		...
		tmp = 0;
		spi_write(st.hw->addr, 0x23, 1, &tmp);
																...
																uint8_t lpa_freq)
{
	...
	if (lpa_freq)
	{
		...
		data[0] = thresh_hw;
		data[1] = time;
		if (spi_write(st.hw->addr, st.reg->motion_thr, 2, data))
			goto lp_int_restore;
		...
		if (spi_write(st.hw->addr, st.reg->user_ctrl, 3, data))
			goto lp_int_restore;

		/* Set motion threshold. */
		data[0] = thresh_hw;
		if (spi_write(st.hw->addr, st.reg->motion_thr, 1, data))
			goto lp_int_restore;
		...
		if (spi_write(st.hw->addr, st.reg->lp_accel_odr, 1, data))
			goto lp_int_restore;

		/* Enable motion interrupt (MPU6500 version). */
		data[0] = BITS_WOM_EN;
		if (spi_write(st.hw->addr, st.reg->accel_intel, 1, data))
			goto lp_int_restore;
